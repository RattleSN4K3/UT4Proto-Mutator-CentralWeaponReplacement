class TestCWRCore extends Mutator
...
{
	return class'TestCWRUI'.static.GetMutatorName(class'TestCentralWeaponReplacement');
}
 ...
 * @return true if newly created
 */
static function bool EnsureMutator(WorldInfo WI, out TestCWRCore OutMut, optional bool NoCreation)
{
	// try to find an existent gneric mutator
	HasMutator(WI, OutMut);

	// if not mutator was found, create one
	if (OutMut == none && !NoCreation)
	{
		OutMut = WI.Spawn(class'TestCWRCore', WI.Game);
		if (WI.Game != none)
		{
...
{
	local TestCWRCore ContextObj;
	local bool IsStatic;
	ContextObj = TestCWRCore(Context);
...
static function bool HasMutator(WorldInfo WI, out TestCWRCore OutMut, optional Object IgnoreMutator)
{
	foreach WI.DynamicActors(class'TestCWRCore', OutMut)
	{
...
private static function bool GetStaticMutator(out TestCWRCore OutMutator)
{
	local Object Obj;
	if (GetDefaultObject(default.Class, Obj) && TestCWRCore(Obj) != none)
	{
		OutMutator = TestCWRCore(Obj);
	}
